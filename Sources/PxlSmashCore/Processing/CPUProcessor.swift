...
public enum CPUProcessor {
    ...
    public static func resize(_ image: CGImage, to size: ResizeSpec) throws -> CGImage {
        ...
        guard srcWidth > 0 && srcHeight > 0 else {
            throw PxlSmashError.invalidInput("Invalid image dimensions: \(srcWidth)x\(srcHeight)")
        }
        ...
        guard let srcContext = CGContext(
            ...
            throw PxlSmashError.generalError("Failed to create source context for resize")
        }
        ...
        guard error == kvImageNoError else {
            throw PxlSmashError.generalError("vImage resize failed with error \(error)")
        }
        ...
        guard let dstContext = CGContext(
            ...
            throw PxlSmashError.generalError("Failed to create destination context for resize")
        }

        guard let result = dstContext.makeImage() else {
            throw PxlSmashError.generalError("Failed to extract resized image")
        }
